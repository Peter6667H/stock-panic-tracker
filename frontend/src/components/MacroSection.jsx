...
const SIGNAL_MSG = {
  ...
  '^TNX':     { dn: '收益率上升，融资成本加剧，成长股承压', up: '收益率下行，利好科技估值' },
...
function MacroTile({ q }) {
  if (!q) return <div className="macro-tile macro-tile-empty">—</div>
  const up = q.pct >= 0
  const sig = SIGNAL_MSG[q.symbol]
  return (
    <div className={`macro-tile${up ? ' macro-up' : ' macro-dn'}`} title={sig ? (up ? sig.up : sig.dn) : ''}>
      <div className="macro-sym">{q.symbol.replace('^', '').replace('=X', '')}</div>
      ...
      {sig && <div className="macro-sig">{up ? sig.up : sig.dn}</div>}
    </div>
  )
}
...
export default function MacroSection() {
  ...
  return (
    <div className="macro-wrap">
      {Object.entries(data.groups).map(([key, syms]) => (
        <div key={key} className="macro-group">
          <div className="macro-group-label">{GROUP_LABELS[key] || key}</div>
          <div className="macro-tiles">
            {syms.map(sym => <MacroTile key={sym} q={data.quotes[sym]} />)}
          </div>
        </div>
      ))}
      <div className="macro-footer">
        <span>数据来源 Yahoo Finance · {age}s 前更新</span>
        <span>^TNX = 10年期美债收益率（越高 = 融资成本越贵，科技股承压）</span>
